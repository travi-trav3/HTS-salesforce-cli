...
# The premob checklist is authored against a full-lead window equal to the
# largest task offset below. When a Work Order is awarded inside that window
# (mobilization is fewer than MAX_OFFSET days out), offsets are compressed
# proportionally so every task still lands between today and the mob date,
# and clamped so nothing is ever created already past due.
MAX_OFFSET = max(t[2] for t in TASKS)
...
def task_block(idx: int, task) -> str:
    ...
    assign_xml = f'''    <assignments>
        ...
        <assignmentItems>
            <assignToReference>{var_name}.Customer_Site__c</assignToReference>
            <operator>Assign</operator>
            <value><elementReference>$Record.Customer_Site__c</elementReference></value>
        </assignmentItems>
        <assignmentItems>
            <assignToReference>{var_name}.Job_Code__c</assignToReference>
            <operator>Assign</operator>
            <value><elementReference>$Record.Job_Code__c</elementReference></value>
        </assignmentItems>
        <assignmentItems>
...
<Flow xmlns="http://soap.sforce.com/2006/04/metadata">
    ...
    <description>Record-triggered flow on Project__c (Work Order) create. Generates the canonical Pre-Mob task checklist as Task records related to the Work Order, with owners, gate flags, and the parent Customer Site and Job Code stamped onto each task. Due dates are offset back from Mobilization_Date__c; when the job is awarded inside the premob window the offsets are compressed proportionally and clamped so no task is ever created already past due. Conditional tasks gate on Requires_New_Hires__c and Method_of_Mobilization__c. Generated by scripts/generate_premob_flow.py.</description>
