...
echo ""
echo "  --- Sprint batch (premob/audit/PO/list-views/dashboard) follow-ups ---"
echo " 10. Confirm active Flow versions after the flow deploy (guardrail):"
echo "       sf data query --target-org $ORG_ALIAS \\"
echo "         --query \"SELECT DeveloperName, VersionNumber, Status FROM Flow WHERE DeveloperName IN ('Generate_PreMob_Tasks','PO_Low_Balance_Alert','Stamp_Task_Completion') AND Status='Active'\" --use-tooling-api"
echo "     The corrected Generate_PreMob_Tasks must be the Active version; deactivate any"
echo "     stale prior version (Setup -> Flows -> Generate Pre-Mob Tasks -> deactivate old version)."
echo " 11. Task fields are created MANUALLY in Setup (Object Manager -> Activity ->"
echo "     Fields & Relationships). This org's Metadata API rejects new custom fields on"
echo "     Task/Activity ('bad value for restricted picklist field: Task'), so deploy step"
echo "     3b is expected to fail and is non-fatal. Required custom fields: Is_Gate__c,"
echo "     Pre_Mob_Section__c, Overdue_Alert_Sent__c, Escalated__c, Completed_By__c (Lookup"
echo "     User), Completed_Date__c (DateTime), Customer_Site__c (Text 255), Job_Code__c"
echo "     (Text 80). Then add Completed By/Date (+ Customer Site / Job Code) to the Task"
echo "     page layout and the Work Order (Project__c) Activity related list."
echo " 12. Task list views are managed in the UI (the Metadata API rejects standard Task"
echo "     list-view column/filter tokens here). The live views are 'Pre-Mob Tasks' and"
echo "     'Pre-Mob Tasks: Next 7 Days' (Setup is the system of record; no list-view"
echo "     metadata is tracked in the repo). 'Next 7 Days' filters Due Date TODAY..NEXT 7 DAYS."
